rm(list = ls())
# ROMS example:
# Explore ROMS outputs:

setwd('C:/Users/moroncog/Documents/DisMELS_Pcod_model')

library(chron)
library(RColorBrewer)
library(lattice)
library(ncdf4)
require(raster)
library(gapminder)
library(ggplot2)
library(wesanderson)
library(magick)
require(ggplot2)
library(plotly)
library(plyr)
library(dplyr)
require(data.table)
library(mapdata)
library(marmap)
library(tidyverse)
require(mapproj)
require(reshape2)
library(gganimate)
require(lubridate)
require(scales)
require(rnaturalearth)
require(rnaturalearthdata)
library(gridExtra)
require(sf)
source('aux_functions.R')
#load('BathyData.RData')
bathy1 = read.csv('main_files/bathy1.csv')
bathy2 = read.csv('main_files/bathy2.csv')

# Read bathymetry information ---------------------------------------------

ak = map_data('worldHires','USA:Alaska')
world = ne_countries(scale = "medium", returnclass = "sf")
ini_number = 8E+6 # check this number in initial data file csv. 
maxHindYear = 2012
scenarioLevels = c("GFDL_rcp85", "GFDL_rcp45", 'hindcast')
mainPal = 'Set1'

# Read output files -------------------------------------------------------
# Here I will read every results file and get the appropiate format to make every figure.
# This approach is better since files will be super large for the forcast period and 1 h time step

thLow1 = 0.025
thHigh1 = 0.975
thLow2 = 0.25
thHigh2 = 0.75

# Read forecast files: ----

main_folder = 'Forecast'
scenarios = list.files(main_folder)
plot_data_0 = list()
plot_data_1 = list()
plot_data_2 = list()
plot_data_3 = list()
plot_data_4 = list()
plot_data_5 = list()
plot_data_6 = list()
plot_data_7 = list()
plot_data_8 = list()
plot_data_9 = list()
plot_data_10 = list()
plot_data_11 = list()
plot_data_12 = list()
plot_data_13 = list()
mort_data = list()
spatdat = list()
distdat = list()
distdat_id = list()
direct_id = list()
end_locations = list()
plot_anom_1 = list()
plot_anom_2 = list()
plot_anom_3 = list()
plot_anom_4 = list()
indList = 1

for(k in seq_along(scenarios)) {

  cores = list.files(path = file.path(main_folder, scenarios[k]))
  
  for(i in seq_along(cores)) {
  
    core_name = cores[i]
    files_core = list.files(path = file.path(main_folder, scenarios[k], core_name))
    fcore = grep(pattern = 'Results_files', x = list.files(path = file.path(main_folder, scenarios[k], core_name)))
    allYears = as.numeric(str_split(string = files_core[fcore], pattern = '_', simplify = TRUE)[,3])
    fcoreNew = fcore[allYears <= maxHindYear]
    
    for(j in seq_along(fcoreNew)) {
      
      tmpData = read_data_in(eggInclude = FALSE, 
                             path = file.path(main_folder, scenarios[k], core_name, files_core[fcoreNew[j]]))
      tmpData$ageYSLround = round(tmpData$ageYSL)

      # Base initial points:
      base_locs = tmpData[tmpData[ , .I[which.min(time)], by = id]$V1]
      base_locs$horizPos1 = ifelse(test = base_locs$horizPos1 > 0, yes = base_locs$horizPos1 - 360, 
                                     no = base_locs$horizPos1)
      base_locs$horizPos1 = base_locs$horizPos1 + 360
      
      # Prepare data for plot 0: # ind passed to FDL
      fdl_ind = unique(tmpData[tmpData$typeName == 'FDL', 'id'])
      fdata = data.frame(year = unique(tmpData$year), surv_id = fdl_ind$id, scenario = scenarios[k])
      plot_data_0[[indList]] = fdata      

      # Find initial and final points (only alive id)
      tmpData2 = tmpData[tmpData$id %in% fdl_ind$id, ]
      init_points = tmpData2[tmpData2[ , .I[which.min(time)], by = id]$V1]
      end_points = tmpData2[tmpData2[ , .I[which.max(time)], by = id]$V1]
      
      # Prepare data for plot 1: Hatching success
      num_data = aggregate(x = list(number = tmpData$number), list(year = tmpData$year, 
                                                                   stage = tmpData$typeName, id = tmpData$id), 
                           FUN = function(x) mean(x))
      num_dataYSL = num_data[num_data$stage == 'YSL', ]
      num_dataYSL$hatsuc = num_dataYSL$number/ini_number
      # Prepare data to save:
      sel_var = 'hatsuc'
      toPlotData = num_dataYSL
      prevAnom = data.frame(toPlotData[,c('year', 'id', sel_var)])
      colnames(prevAnom)[3] = 'value'
      prevAnom$scenario = scenarios[k]
      prevAnom$value2 = scales::rescale(x = prevAnom$value, to = c(-1,1))
      plot_anom_1[[indList]] = prevAnom
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                        FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      plot_data_1[[indList]] = fdata
      
      # Prepare data for plot 2: Survival 30 dph (only alive ID)
      survData = tmpData[tmpData$ageYSLround == 30, ]
      surv_data = aggregate(x = list(psurv = survData$psurvival), list(year = survData$year, id = survData$id),
                            FUN = mean, na.rm = TRUE)
      # Prepare data to save:
      sel_var = 'psurv'
      toPlotData = surv_data
      prevAnom = data.frame(toPlotData[,c('year', 'id', sel_var)])
      prevAnom$scenario = scenarios[k]
      colnames(prevAnom)[3] = 'value'
      prevAnom$value2 = scales::rescale(x = prevAnom$value, to = c(-1,1))
      plot_anom_2[[indList]] = prevAnom
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      plot_data_2[[indList]] = fdata

      # Prepare data for plot 8: environmental variables
      stageData = tmpData[tmpData$typeName %in% c('YSL', 'FDL', 'FDLpf', 'Epijuv'), ] 
      env_data = aggregate(x = list(temperature = stageData$temp, pCO2 = stageData$pCO2val), 
                           list(year = stageData$year, id = stageData$id), 
                           FUN = mean, na.rm=TRUE)
      int_data = gather(env_data, key = "variable", value = "value", temperature, pCO2)
      # Prepare data to save:
      sel_var = 'value'
      toPlotData = int_data
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                      FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, variable = p50$variable, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      plot_data_8[[indList]] = fdata
      
      # Prepare data for plot 9: prey abundance
      prey_data = aggregate(x = list(copepods = stageData$copepod, microzoo = stageData$microzoo, 
                                     neocalanus = stageData$neocalanus, neocalanusShelf = stageData$neocalanusShelf, 
                                     euphausiids = stageData$euphausiid, euphausiidsShelf = stageData$euphausiidShelf), 
                            list(year = stageData$year, id = stageData$id), 
                            FUN = mean, na.rm=TRUE)
      int_data = gather(prey_data, key = "variable", value = "value",
                        copepods, microzoo, neocalanus, neocalanusShelf, euphausiids, euphausiidsShelf)
      # Prepare data to save:
      sel_var = 'value'
      toPlotData = int_data
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                      FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, variable = p50$variable, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      plot_data_9[[indList]] = fdata
      
      # Mortality:
      myData = tmpData[(tmpData$ageYSLround <= 30), ]
      myData2 = myData[(myData$progYSA >= 1 | is.na(myData$progYSA)), ]
      myData = myData[which(myData$mortstarv < 10), ]
      myData2 = myData2[which(myData2$mortstarv < 10), ]
      
      myData3 = aggregate(list(mortfish = myData$mortfish, mortinv = myData$mortinv), 
                          list(year = myData$year, id = myData$id), FUN = mean)
      myData4 = myData3[,-2]
      myData5 = myData4 %>% 
        gather('type', 'value', -year)
      #Find prop starv:
      myData6 = aggregate(list(value = myData2$mortstarv), 
                          list(year = myData2$year, id = myData2$id), FUN = function(x) sum(x)/length(x))
      myData6$id = 'mortstarv'
      colnames(myData6)[2] = 'type'
      
      myData7 = rbind(myData5, myData6)
      # Prepare data to save:
      sel_var = 'value'
      toPlotData = myData7
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                      FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, type = p50$type, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      mort_data[[indList]] = fdata
      
      # Prepare data for plot 3: diet rank
      rank_data = aggregate(x = list(rank = tmpData2$avgRank), list(year = tmpData2$year, 
                                                                   stage = tmpData2$typeName, id = tmpData2$id), 
                            FUN = mean, na.rm=TRUE)
      rank_data = rank_data[!(rank_data$stage %in% c('YSL')), ] # exclude YSL and benthicjuv stage
      # Prepare data to save:
      sel_var = 'rank'
      toPlotData = rank_data
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                      FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, stage = p50$stage, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      plot_data_3[[indList]] = fdata
  
      
      # Prepare data for plot 4: diet size
      size_data = aggregate(x = list(sized = tmpData2$avgSize), list(year = tmpData2$year, stage = 
                                                                      tmpData2$typeName, id = tmpData2$id), 
                            FUN = mean, na.rm=TRUE)
      size_data = size_data[!(size_data$stage %in% c('YSL')), ] # exclude YSL and benthicjuv stage
      # Prepare data to save:
      sel_var = 'sized'
      toPlotData = size_data
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                      FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, stage = p50$stage, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      plot_data_4[[indList]] = fdata

      # -------------------------------------------
      # Delete dead individuals from here: based on DW max
      # delRows = which(tmpData2$DW < 0.75*tmpData2$dwmax) # remove ind dead.
      # tmpData = tmpData[-delRows, ]
      
      # Prepare data for plot 5: DW
      epijuvData = tmpData2[tmpData2$typeName == 'Epijuv', ]
      wgt_data = aggregate(x = list(dw = epijuvData$DW), list(year = epijuvData$year, id = epijuvData$id), 
                           FUN = max, na.rm=TRUE)
      # Prepare data to save:
      sel_var = 'dw'
      toPlotData = wgt_data
      prevAnom = data.frame(toPlotData[,c('year', 'id', sel_var)])
      colnames(prevAnom)[3] = 'value'
      prevAnom$scenario = scenarios[k]
      prevAnom$value2 = scales::rescale(x = prevAnom$value, to = c(-1,1))
      plot_anom_4[[indList]] = prevAnom
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      plot_data_5[[indList]] = fdata

      # Prepare data for plot 6: SL
      sl_data = aggregate(x = list(sl = epijuvData$SL), list(year = epijuvData$year, id = epijuvData$id), 
                          FUN = max, na.rm=TRUE)
      # Prepare data to save:
      sel_var = 'sl'
      toPlotData = sl_data
      prevAnom = data.frame(toPlotData[,c('year', 'id', sel_var)])
      colnames(prevAnom)[3] = 'value'
      prevAnom$scenario = scenarios[k]
      prevAnom$value2 = scales::rescale(x = prevAnom$value, to = c(-1,1))
      plot_anom_3[[indList]] = prevAnom
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      plot_data_6[[indList]] = fdata
      
      # Prepare data for plot 7: final points map
      # epijuvData$last_day = paste0(day(epijuvData$time), '-', month(epijuvData$time))
      # epijuvData2 = epijuvData[epijuvData$last_day == '2-10', ] # final date
      # plot_data_7[[indList]] = epijuvData2[,c('horizPos1', 'horizPos2', 'year')]
      

      # These plots are based on release day:
      # Prepare data for plot 10: hatching success by release day
      # tmpData$relDay = lubridate::day(tmpData$startTime)
      # num_data = aggregate(x = list(number = tmpData$number), list(relDay = tmpData$relDay, year = tmpData$year,
      #                                                              stage = tmpData$typeName, id = tmpData$id), 
      #                      FUN = mean, na.rm = TRUE)
      # num_dataYSL = num_data[num_data$stage == 'YSL', ]
      # num_dataYSL$hatsuc = num_dataYSL$number/ini_number
      # plot_data_10[[indList]] = num_dataYSL
      # 
      # # Prepare data for plot 11: survival 30dph by release day
      # survData$relDay = lubridate::day(survData$startTime)
      # surv_data = aggregate(x = list(psurv = survData$psurvival), 
      #                       list(relDay = survData$relDay, id = survData$id, year = survData$year),
      #                       FUN = mean, na.rm = TRUE)
      # plot_data_11[[indList]] = surv_data
      # 
      #   # Prepare data for plot 12: SL by release day
      # epijuvData = tmpData[tmpData$typeName == 'Epijuv', ]
      # epijuvData$relDay = lubridate::day(epijuvData$startTime)
      # sl_data = aggregate(x = list(sl = epijuvData$SL), list(relDay = epijuvData$relDay, 
      #                                                         id = epijuvData$id), 
      #                      FUN = max, na.rm=TRUE)
      # plot_data_12[[indList]] = sl_data
      # 
      # # Prepare data for plot 13: DW by release day
      # wgt_data = aggregate(x = list(dw = epijuvData$DW), list(relDay = epijuvData$relDay, 
      #                                                         id = epijuvData$id), 
      #                      FUN = max, na.rm=TRUE)
      # plot_data_13[[indList]] = wgt_data
          
      # Calculate distance per id:
      mergePoints = rbind(init_points[,c('horizPos1', 'horizPos2', 'id')],
                          end_points[,c('horizPos1', 'horizPos2', 'id')])
      distMat = mergePoints %>%
        group_by(id)%>%
        group_map(~raster::pointDistance(.x[,c('horizPos1', 'horizPos2')], lonlat=TRUE)) %>%
        setNames(unique(sort(mergePoints$id)))
      distVals = unlist(distMat)
      distVals = distVals[distVals > 0 & !is.na(distVals)]
      distValsNm = (distVals/111000)*60 # units: nm
      preDist = data.frame(dist = distValsNm, year = unique(tmpData$year)[1])
      preDist$id = as.numeric(names(distMat))
      # Prepare data to save:
      sel_var = 'dist'
      toPlotData = preDist
      p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = median)
      p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                       FUN = quantile, probs = thLow1, na.rm = TRUE)
      p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                        FUN = quantile, probs = thHigh1, na.rm = TRUE)
      p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thLow2, na.rm = TRUE)
      p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thHigh2, na.rm = TRUE)
      fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                         q5 = p2_5$var, q95 = p97_5$var, scenario = scenarios[k])
      distdat[[indList]] = fdata
      preDist$scenario = scenarios[k]
      distdat_id[[indList]] = preDist
      
      # Calculate direction
      direcMat = mergePoints %>%
        group_by(id)%>%
        group_map(~geosphere::bearing(.x[,c('horizPos1', 'horizPos2')])) %>%
        setNames(unique(sort(mergePoints$id)))
      direcVals = unlist(direcMat)
      direcVals = direcVals[!is.na(direcVals)]
      direcValsNm = ifelse(test = direcVals < 0, yes = 360+direcVals, no = direcVals)
      preDirec = data.frame(dist = direcValsNm, year = unique(tmpData$year)[1])
      preDirec$id = as.numeric(names(direcMat))
      preDirec$scenario = scenarios[k]
      direct_id[[indList]] = preDirec
      
      # Calculate CG and Inertia:
      init_points$horizPos1 = ifelse(test = init_points$horizPos1 > 0, yes = init_points$horizPos1 - 360, 
                                 no = init_points$horizPos1)
      init_points$horizPos1 = init_points$horizPos1 + 360
      init_points$relDay = lubridate::day(init_points$startTime)
      
      end_points$horizPos1 = ifelse(test = end_points$horizPos1 > 0, yes = end_points$horizPos1 - 360, 
                                     no = end_points$horizPos1)
      end_points$horizPos1 = end_points$horizPos1 + 360
      end_points$relDay = lubridate::day(end_points$startTime)
      
      spatInfoIni = init_points %>%
        group_by(year)%>%
        group_map(~cgi(x = .x$horizPos1, y = .x$horizPos2))
      spatInfoEnd = end_points %>%
        group_by(year)%>%
        group_map(~cgi(x = .x$horizPos1, y = .x$horizPos2))
      spatdat[[indList]] = data.frame(CG_x_ini = sapply(spatInfoIni, "[[", 1), 
                                      CG_x_end = sapply(spatInfoEnd, "[[", 1),
                                      CG_y_ini = sapply(spatInfoIni, "[[", 2),
                                      CG_y_end = sapply(spatInfoEnd, "[[", 2),
                                      I_ini = sapply(spatInfoIni, "[[", 3), 
                                      I_end = sapply(spatInfoEnd, "[[", 3),
                                      year = unique(tmpData$year)[1],
                                      scenario = scenarios[k])
      
      # Save final locations:
      endLocs_df = end_points[,c('id', 'horizPos1', 'horizPos2', 'year')] 
      endLocs_df$scenario = scenarios[k]
      end_locations[[indList]] = endLocs_df  
      
      
      # Get to next indicator:
      print(indList)
      indList = indList + 1
      
    }
    
  }

}


# Read hindcast files:

main_folder = 'Hindcast'
scenario = 'hindcast'
plot_data_h0 = list()
plot_data_h1 = list()
plot_data_h2 = list()
plot_data_h3 = list()
plot_data_h4 = list()
plot_data_h5 = list()
plot_data_h6 = list()
plot_data_h7 = list()
plot_data_h8 = list()
plot_data_h9 = list()
mort_datah = list()
spatdath = list()
distdath = list()
distdat_idh = list()
direct_idh = list()
end_locationsh = list()
plot_anom_h1 = list()
plot_anom_h2 = list()
plot_anom_h3 = list()
plot_anom_h4 = list()
indList = 1

cores = list.files(path = main_folder)

for(i in seq_along(cores)) {
  
  core_name = cores[i]
  files_core = list.files(path = file.path(main_folder, core_name))
  fcore = grep(pattern = 'Results_files', x = list.files(path = file.path(main_folder, core_name)))

  for(j in seq_along(fcore)) {
    
    tmpData = read_data_in(eggInclude = FALSE, 
                           path = file.path(main_folder, core_name, files_core[fcore[j]]))
    tmpData$ageYSLround = round(tmpData$ageYSL)
    
    # Base initial points:
    base_locs = tmpData[tmpData[ , .I[which.min(time)], by = id]$V1]
    base_locs$horizPos1 = ifelse(test = base_locs$horizPos1 > 0, yes = base_locs$horizPos1 - 360, 
                                 no = base_locs$horizPos1)
    base_locs$horizPos1 = base_locs$horizPos1 + 360
    
    # Prepare data for plot 0: # ind passed to FDL
    fdl_ind = unique(tmpData[tmpData$typeName == 'FDL', 'id'])
    fdata = data.frame(year = unique(tmpData$year), surv_id = fdl_ind$id, scenario = scenario)
    plot_data_h0[[indList]] = fdata      
    
    # Find initial and final points (only alive id)
    tmpData2 = tmpData[tmpData$id %in% fdl_ind$id, ]
    init_points = tmpData2[tmpData2[ , .I[which.min(time)], by = id]$V1]
    end_points = tmpData2[tmpData2[ , .I[which.max(time)], by = id]$V1]
    
    # Prepare data for plot 1: Hatching success
    num_data = aggregate(x = list(number = tmpData$number), list(year = tmpData$year, 
                                                                 stage = tmpData$typeName, id = tmpData$id), 
                         FUN = function(x) mean(x))
    num_dataYSL = num_data[num_data$stage == 'YSL', ]
    num_dataYSL$hatsuc = num_dataYSL$number/ini_number
    # Prepare data to save:
    sel_var = 'hatsuc'
    toPlotData = num_dataYSL
    prevAnom = data.frame(toPlotData[,c('year', 'id', sel_var)])
    colnames(prevAnom)[3] = 'value'
    prevAnom$scenario = scenario
    prevAnom$value2 = scales::rescale(x = prevAnom$value, to = c(-1,1))
    plot_anom_h1[[indList]] = prevAnom
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    plot_data_h1[[indList]] = fdata
    
    # Prepare data for plot 2: Survival 30 dph (only alive ID)
    survData = tmpData[tmpData$ageYSLround == 30, ]
    surv_data = aggregate(x = list(psurv = survData$psurvival), list(year = survData$year, id = survData$id),
                          FUN = mean, na.rm = TRUE)
    # Prepare data to save:
    sel_var = 'psurv'
    toPlotData = surv_data
    prevAnom = data.frame(toPlotData[,c('year', 'id', sel_var)])
    prevAnom$scenario = scenario
    colnames(prevAnom)[3] = 'value'
    prevAnom$value2 = scales::rescale(x = prevAnom$value, to = c(-1,1))
    plot_anom_h2[[indList]] = prevAnom
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    plot_data_h2[[indList]] = fdata
    
    # Prepare data for plot 8: environmental variables
    stageData = tmpData[tmpData$typeName %in% c('YSL', 'FDL', 'FDLpf', 'Epijuv'), ] 
    env_data = aggregate(x = list(temperature = stageData$temp, pCO2 = stageData$pCO2val), 
                         list(year = stageData$year, id = stageData$id), 
                         FUN = mean, na.rm=TRUE)
    int_data = gather(env_data, key = "variable", value = "value", temperature, pCO2)
    # Prepare data to save:
    sel_var = 'value'
    toPlotData = int_data
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, variable = p50$variable, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    plot_data_h8[[indList]] = fdata
    
    # Prepare data for plot 9: prey abundance
    prey_data = aggregate(x = list(copepods = stageData$copepod, microzoo = stageData$microzoo, 
                                   neocalanus = stageData$neocalanus, neocalanusShelf = stageData$neocalanusShelf, 
                                   euphausiids = stageData$euphausiid, euphausiidsShelf = stageData$euphausiidShelf), 
                          list(year = stageData$year, id = stageData$id), 
                          FUN = mean, na.rm=TRUE)
    int_data = gather(prey_data, key = "variable", value = "value",
                      copepods, microzoo, neocalanus, neocalanusShelf, euphausiids, euphausiidsShelf)
    # Prepare data to save:
    sel_var = 'value'
    toPlotData = int_data
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, variable = toPlotData$variable), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, variable = p50$variable, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    plot_data_h9[[indList]] = fdata
    
    # Mortality:
    myData = tmpData[(tmpData$ageYSLround <= 30), ]
    myData2 = myData[(myData$progYSA >= 1 | is.na(myData$progYSA)), ]
    myData = myData[which(myData$mortstarv < 10), ]
    myData2 = myData2[which(myData2$mortstarv < 10), ]
    
    myData3 = aggregate(list(mortfish = myData$mortfish, mortinv = myData$mortinv), 
                        list(year = myData$year, id = myData$id), FUN = mean)
    myData4 = myData3[,-2]
    myData5 = myData4 %>% 
      gather('type', 'value', -year)
    #Find prop starv:
    myData6 = aggregate(list(value = myData2$mortstarv), 
                        list(year = myData2$year, id = myData2$id), FUN = function(x) sum(x)/length(x))
    myData6$id = 'mortstarv'
    colnames(myData6)[2] = 'type'
    
    myData7 = rbind(myData5, myData6)
    # Prepare data to save:
    sel_var = 'value'
    toPlotData = myData7
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, type = toPlotData$type), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, type = p50$type, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    mort_datah[[indList]] = fdata
    
    # Prepare data for plot 3: diet rank
    rank_data = aggregate(x = list(rank = tmpData2$avgRank), list(year = tmpData2$year, 
                                                                  stage = tmpData2$typeName, id = tmpData2$id), 
                          FUN = mean, na.rm=TRUE)
    rank_data = rank_data[!(rank_data$stage %in% c('YSL')), ] # exclude YSL and benthicjuv stage
    # Prepare data to save:
    sel_var = 'rank'
    toPlotData = rank_data
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, stage = p50$stage, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    plot_data_h3[[indList]] = fdata
    
    
    # Prepare data for plot 4: diet size
    size_data = aggregate(x = list(sized = tmpData2$avgSize), list(year = tmpData2$year, stage = 
                                                                     tmpData2$typeName, id = tmpData2$id), 
                          FUN = mean, na.rm=TRUE)
    size_data = size_data[!(size_data$stage %in% c('YSL')), ] # exclude YSL and benthicjuv stage
    # Prepare data to save:
    sel_var = 'sized'
    toPlotData = size_data
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year, stage = toPlotData$stage), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, stage = p50$stage, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    plot_data_h4[[indList]] = fdata
    
    # -------------------------------------------
    # Prepare data for plot 5: DW
    epijuvData = tmpData2[tmpData2$typeName == 'Epijuv', ]
    wgt_data = aggregate(x = list(dw = epijuvData$DW), list(year = epijuvData$year, id = epijuvData$id), 
                         FUN = max, na.rm=TRUE)
    # Prepare data to save:
    sel_var = 'dw'
    toPlotData = wgt_data
    prevAnom = data.frame(toPlotData[,c('year', 'id', sel_var)])
    colnames(prevAnom)[3] = 'value'
    prevAnom$scenario = scenario
    prevAnom$value2 = scales::rescale(x = prevAnom$value, to = c(-1,1))
    plot_anom_h4[[indList]] = prevAnom
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    plot_data_h5[[indList]] = fdata
    
    # Prepare data for plot 6: SL
    sl_data = aggregate(x = list(sl = epijuvData$SL), list(year = epijuvData$year, id = epijuvData$id), 
                        FUN = max, na.rm=TRUE)
    # Prepare data to save:
    sel_var = 'sl'
    toPlotData = sl_data
    prevAnom = data.frame(toPlotData[,c('year', 'id', sel_var)])
    colnames(prevAnom)[3] = 'value'
    prevAnom$scenario = scenario
    prevAnom$value2 = scales::rescale(x = prevAnom$value, to = c(-1,1))
    plot_anom_h3[[indList]] = prevAnom
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    plot_data_h6[[indList]] = fdata
    
    # Calculate distance per id:
    mergePoints = rbind(init_points[,c('horizPos1', 'horizPos2', 'id')],
                        end_points[,c('horizPos1', 'horizPos2', 'id')])
    distMat = mergePoints %>%
      group_by(id)%>%
      group_map(~raster::pointDistance(.x[,c('horizPos1', 'horizPos2')], lonlat=TRUE)) %>%
      setNames(unique(sort(mergePoints$id)))
    distVals = unlist(distMat)
    distVals = distVals[distVals > 0 & !is.na(distVals)]
    distValsNm = (distVals/111000)*60 # units: nm
    preDist = data.frame(dist = distValsNm, year = unique(tmpData$year)[1])
    preDist$id = as.numeric(names(distMat))
    # Prepare data to save:
    sel_var = 'dist'
    toPlotData = preDist
    p50 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = median)
    p2_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                     FUN = quantile, probs = thLow1, na.rm = TRUE)
    p97_5 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                      FUN = quantile, probs = thHigh1, na.rm = TRUE)
    p25 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thLow2, na.rm = TRUE)
    p75 = aggregate(list(var = toPlotData[,sel_var]), list(year = toPlotData$year), 
                    FUN = quantile, probs = thHigh2, na.rm = TRUE)
    fdata = data.frame(year = p50$year, q50 = p50$var, q25 = p25$var, q75 = p75$var,
                       q5 = p2_5$var, q95 = p97_5$var, scenario = scenario)
    distdath[[indList]] = fdata
    preDist$scenario = scenario
    distdat_idh[[indList]] = preDist
    
    # Calculate direction
    direcMat = mergePoints %>%
      group_by(id)%>%
      group_map(~geosphere::bearing(.x[,c('horizPos1', 'horizPos2')])) %>%
      setNames(unique(sort(mergePoints$id)))
    direcVals = unlist(direcMat)
    direcVals = direcVals[!is.na(direcVals)]
    direcValsNm = ifelse(test = direcVals < 0, yes = 360+direcVals, no = direcVals)
    preDirec = data.frame(dist = direcValsNm, year = unique(tmpData$year)[1])
    preDirec$id = as.numeric(names(direcMat))
    preDirec$scenario = scenario
    direct_idh[[indList]] = preDirec
    
    # Calculate CG and Inertia:
    init_points$horizPos1 = ifelse(test = init_points$horizPos1 > 0, yes = init_points$horizPos1 - 360, 
                                   no = init_points$horizPos1)
    init_points$horizPos1 = init_points$horizPos1 + 360
    init_points$relDay = lubridate::day(init_points$startTime)
    
    end_points$horizPos1 = ifelse(test = end_points$horizPos1 > 0, yes = end_points$horizPos1 - 360, 
                                  no = end_points$horizPos1)
    end_points$horizPos1 = end_points$horizPos1 + 360
    end_points$relDay = lubridate::day(end_points$startTime)
    
    spatInfoIni = init_points %>%
      group_by(year)%>%
      group_map(~cgi(x = .x$horizPos1, y = .x$horizPos2))
    spatInfoEnd = end_points %>%
      group_by(year)%>%
      group_map(~cgi(x = .x$horizPos1, y = .x$horizPos2))
    spatdath[[indList]] = data.frame(CG_x_ini = sapply(spatInfoIni, "[[", 1), 
                                    CG_x_end = sapply(spatInfoEnd, "[[", 1),
                                    CG_y_ini = sapply(spatInfoIni, "[[", 2),
                                    CG_y_end = sapply(spatInfoEnd, "[[", 2),
                                    I_ini = sapply(spatInfoIni, "[[", 3), 
                                    I_end = sapply(spatInfoEnd, "[[", 3),
                                    year = unique(tmpData$year)[1],
                                    scenario = scenario)
    
    # Save final locations:
    endLocs_df = end_points[,c('id', 'horizPos1', 'horizPos2', 'year')] 
    endLocs_df$scenario = scenario
    end_locationsh[[indList]] = endLocs_df  
    
    
    # Get to next indicator:
    print(indList)
    indList = indList + 1
    
  }
  
}


# Info required for future plots:
baseLocs = base_locs[,c('id', 'horizPos1', 'horizPos2')]
baseLocs = baseLocs[order(baseLocs$id), ]
rainbowPal = rev(c(rosso_corsa = "#D12600", spanish_orange = "#DB6A00", 
               green_yellow = "#B2FF2E", green = "#00AD00", pale_cerulean = "#9CCADE", 
               sea_blue = "#005B94", st_patricks_blue = "#1E2085", 
               tyrian_purple = "#610052", amaranth_deep_purple = "#953272"))
rainbowFunc = colorRampPalette(rainbowPal)


# Analyze BY YEAR ---------------------------------------------------------
# -------------------------------------------------------------------------

# Plot 1: Hatching success --------------------------------------------------------

plot_dataf = bind_rows(plot_data_1, .id = "column_label")
plot_datah = bind_rows(plot_data_h1, .id = "column_label")
plot_data = rbind(plot_datah, plot_dataf)
plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)

# Plot:
pl1 = ggplot(plot_data, aes(x = year)) + 
  geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
  #geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
  geom_line(aes(y = q50, colour = scenario)) +
  theme_bw() +
  xlab('') +
  ylab('Hatching success') +
  scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2003, to = maxHindYear, by = 2)) +
  scale_color_brewer(palette = mainPal) +
  scale_fill_brewer(palette = mainPal) +
  theme(legend.position = 'none')

# ggsave(filename = 'figures/hind_hatchsuccess.png', device = 'png', width = 100, height = 75, units = 'mm', dpi = 500)
write.csv(plot_data, 'output_data/forehind_hatchsuc.csv', row.names = FALSE)

# Plot 2: Survival 30 days --------------------------------------------------------

plot_dataf = bind_rows(plot_data_2, .id = "column_label")
plot_datah = bind_rows(plot_data_h2, .id = "column_label")
plot_data = rbind(plot_datah, plot_dataf)
plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)

# Plot:
pl2 = ggplot(plot_data, aes(x = year)) + 
  geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
  #geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
  geom_line(aes(y = q50, colour = scenario)) +
  theme_bw() +
  xlab('') +
  ylab('Survival probability (30 dph)') +
  scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2003, to = maxHindYear, by = 2)) +
  scale_color_brewer(palette = mainPal) +
  scale_fill_brewer(palette = mainPal) +
  theme(legend.position = 'none')

# ggsave(filename = 'figures/hind_survprob.png', device = 'png', width = 100, height = 75, units = 'mm', dpi = 500)
write.csv(plot_data, 'output_data/forehind_surv.csv', row.names = FALSE)

# Plot 2.2: survival rate by category -------------------------------------
# Only fish and invert predation
# plot_data = bind_rows(mort_data, .id = "column_label")
# plot_data_1 = plot_data[plot_data$type %in% c('mortfish', 'mortinv'), ]
# plot_data_1$scenario = factor(plot_data_1$scenario, levels = scenarioLevels)
# plot_data_1$type = factor(plot_data_1$type, levels = c("mortfish", "mortinv"))
# plot_data_1$type = factor(plot_data_1$type, labels = c('Fish predation', 'Invertebrate predation'))
# 
# pm1 = ggplot(plot_data_1, aes(x = year)) + 
#         geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
#         geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
#         geom_line(aes(y = q50, colour = scenario)) +
#         theme_bw() +
#         xlab('') +
#         ylab(expression(paste('Mortality 30 dph (', s^{-1}, ', ', 10^{-6}, ')'))) +
#         scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2015, to = 2095, by = 20)) +
#         scale_color_brewer(palette = mainPal) +
#         scale_fill_brewer(palette = mainPal) +
#         theme(legend.position = 'none') +
#         facet_wrap(~type, ncol = 2, scales = 'free_y')
# 
# write.csv(plot_data_1, 'output_data/fore_mort_pm1.csv', row.names = FALSE)
# 
# # % time steps with starvation:
# plot_data_2 = plot_data[plot_data$type %in% 'mortstarv', ]
# plot_data_2$scenario = factor(plot_data_2$scenario, levels = scenarioLevels)
# 
# pm2 = ggplot(plot_data_2, aes(x = year)) + 
#         geom_ribbon(aes(ymin = q5*100, ymax = q95*100, fill = scenario), alpha=0.1) +
#         geom_ribbon(aes(ymin = q25*100, ymax = q75*100, fill = scenario), alpha=0.2) +
#         geom_line(aes(y = q50*100, colour = scenario)) +
#         theme_bw() +
#         xlab('') +
#         ylab('% time steps with starvation') +
#         scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2015, to = 2095, by = 20)) +
#         scale_color_brewer(palette = mainPal) +
#         scale_fill_brewer(palette = mainPal) +
#         theme(legend.position = 'none') 
# 
# write.csv(plot_data_2, 'output_data/fore_mort_pm2.csv', row.names = FALSE)
# 
# # Num ind that passed to FDL 
# n_id = 807
# plot_data = bind_rows(plot_data_0, .id = "column_label")
# plot_data = aggregate(list(porcsurv = plot_data$surv_id), list(year = plot_data$year, scenario = plot_data$scenario),
#                       FUN = function(x) length(unique(x))/n_id)
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# # Plot:
# pm3 = ggplot(plot_data, aes(x = year, y = porcsurv*100, color = scenario)) + 
#         geom_line() +
#         theme_bw() +
#         xlab('') +
#         ylab('% first feeding') +
#         scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2015, to = 2095, by = 20)) +
#         scale_color_brewer(palette = mainPal) +
#         theme(legend.position = 'none') 
# 
# write.csv(plot_data, 'output_data/fore_mort_pm3.csv', row.names = FALSE)
# 
# 
# lay = rbind(c(1,1),
#              c(2,3))
# png(filename = 'figures/fore_mort_types.png', width = 190, height = 130, 
#     units = 'mm', res = 500)
# grid.arrange(pm1, pm2, pm3, layout_matrix = lay)
# dev.off()
# 
# 
# # Plot 3: Changes rank in diet ---------------------------------------------------------
# 
# plot_data = bind_rows(plot_data_3, .id = "column_label")
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# plot_data$stage = as.factor(plot_data$stage)
# 
# # Plot:
# pd1 = ggplot(plot_data, aes(x = year)) + 
#   geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
#   geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
#   geom_line(aes(y = q50, colour = scenario)) +
#   theme_bw() +
#   xlab('') +
#   ylab('Prey preference') +
#   scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2015, to = 2095, by = 20)) +
#   scale_y_reverse(limits = c(6.5, 0.5), breaks = 1:6, labels = c('EupO', 'EupS', 'NCaS', 'NCaO', 'Cop', 'MZP')) +
#   scale_color_brewer(palette = mainPal) +
#   scale_fill_brewer(palette = mainPal) +
#   theme(legend.position = 'none') +
#   facet_grid(~stage)
# 
# 
# # ggsave(filename = 'figures/hind_dietrank.png', device = 'png', width = 180, height = 75, units = 'mm', dpi = 500)
# write.csv(plot_data, 'output_data/fore_rankdiet.csv', row.names = FALSE)
# 
# 
# # Plot 4: Changes size in diet ---------------------------------------------------------
# 
# plot_data = bind_rows(plot_data_4, .id = "column_label")
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# plot_data$stage = as.factor(plot_data$stage)
# 
# # Plot:
# pd2 = ggplot(plot_data, aes(x = year)) + 
#   geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
#   geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
#   geom_line(aes(y = q50, colour = scenario)) +
#   theme_bw() +
#   xlab('') +
#   ylab('Prey size (mm)') +
#   scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2015, to = 2095, by = 20)) +
#   scale_color_brewer(palette = mainPal) +
#   scale_fill_brewer(palette = mainPal) +
#   theme(legend.position = 'none') +
#   facet_wrap(~stage, scales = 'free_y')
# 
# # ggsave(filename = 'figures/hind_dietsize.png', device = 'png', width = 180, height = 75, units = 'mm', dpi = 500)
# write.csv(plot_data, 'output_data/fore_sizediet.csv', row.names = FALSE)
# 
# # Prey figure:
# png(filename = 'figures/fore_preyfig.png', width = 190, height = 130, 
#     units = 'mm', res = 500)
# grid.arrange(pd1, pd2)
# dev.off()

# Plot 5: Changes in DW ----------------------------------------------------------

plot_dataf = bind_rows(plot_data_5, .id = "column_label")
plot_datah = bind_rows(plot_data_h5, .id = "column_label")
plot_data = rbind(plot_datah, plot_dataf)
plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)

# Plot:
pl4 = ggplot(plot_data, aes(x = year)) + 
  geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
  #geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
  geom_line(aes(y = q50, colour = scenario)) +
  theme_bw() +
  xlab('') +
  ylab('Dry weight (mg)') +
  scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2003, to = maxHindYear, by = 2)) +
  scale_color_brewer(palette = mainPal) +
  scale_fill_brewer(palette = mainPal) +
  theme(legend.position = 'none')


# ggsave(filename = 'figures/hind_dryw.png', device = 'png', width = 100, height = 75, units = 'mm', dpi = 500)
write.csv(plot_data, 'output_data/forehind_DW.csv', row.names = FALSE)

# Plot 6: Changes in SL ----------------------------------------------------------

plot_dataf = bind_rows(plot_data_6, .id = "column_label")
plot_datah = bind_rows(plot_data_h6, .id = "column_label")
plot_data = rbind(plot_datah, plot_dataf)
plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)

# Plot:
pl3 = ggplot(plot_data, aes(x = year)) + 
  geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
  #geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
  geom_line(aes(y = q50, colour = scenario)) +
  theme_bw() +
  xlab('') +
  ylab('Standard length (mm)') +
  scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2003, to = maxHindYear, by = 2)) +
  scale_color_brewer(palette = mainPal) +
  scale_fill_brewer(palette = mainPal) +
  theme(legend.position = 'none')

# ggsave(filename = 'figures/hind_stdlen.png', device = 'png', width = 100, height = 75, units = 'mm', dpi = 500)
write.csv(plot_data, 'output_data/forehind_SL.csv', row.names = FALSE)

# Prey figure:
png(filename = 'figures/forehind_indfig.png', width = 190, height = 130, 
    units = 'mm', res = 500)
grid.arrange(pl1, pl2, pl3, pl4)
dev.off()



# Plot map anomalies ----------------------------------------------------------

# # Hatching success
# plot_data = bind_rows(plot_anom_1, .id = "column_label")
# plot_data = aggregate(list(var = plot_data$value2), list(id = plot_data$id, scenario = plot_data$scenario),
#                       FUN = mean)
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data = plot_data, legTitle = 'Devs', palFun = rainbowFunc)
# 
# ggsave(filename = 'figures/fore_mapanom_hatchsuc.png', device = 'png', width = 100, height = 160, units = 'mm', dpi = 500)
# 
# write.csv(plot_data, 'output_data/fore_anom_hatchsuc.csv', row.names = FALSE)
# 
# # Survival
# plot_data = bind_rows(plot_anom_2, .id = "column_label")
# plot_data = aggregate(list(var = plot_data$value2), list(id = plot_data$id, scenario = plot_data$scenario),
#                       FUN = median)
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data = plot_data, legTitle = 'Devs', palFun = rainbowFunc)
# 
# ggsave(filename = 'figures/fore_mapanom_surv.png', device = 'png', width = 100, height = 160, units = 'mm', dpi = 500)
# 
# write.csv(plot_data, 'output_data/fore_anom_surv.csv', row.names = FALSE)
# 
# # SL
# plot_data = bind_rows(plot_anom_3, .id = "column_label")
# plot_data = aggregate(list(var = plot_data$value2), list(id = plot_data$id, scenario = plot_data$scenario),
#                       FUN = median)
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data = plot_data, legTitle = 'Devs', palFun = rainbowFunc)
# 
# ggsave(filename = 'figures/fore_mapanom_SL.png', device = 'png', width = 100, height = 160, units = 'mm', dpi = 500)
# 
# write.csv(plot_data, 'output_data/fore_anom_SL.csv', row.names = FALSE)
# 
# # DW
# 
# plot_data = bind_rows(plot_anom_4, .id = "column_label")
# plot_data = aggregate(list(var = plot_data$value2), list(id = plot_data$id, scenario = plot_data$scenario),
#                       FUN = median)
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data = plot_data, legTitle = 'Devs', palFun = rainbowFunc)
# 
# ggsave(filename = 'figures/fore_mapanom_DW.png', device = 'png', width = 100, height = 160, units = 'mm', dpi = 500)
# 
# write.csv(plot_data, 'output_data/fore_anom_DW.csv', row.names = FALSE)
# 
# 
# # -------------------------------------------------------------------------
# # Plot spatiotemporal trends (slope):
# 
# # Hatch success:
# plot_data = bind_rows(plot_anom_1, .id = "column_label")
# mods = dlply(plot_data, c("id", "scenario"), function(df) {
#   mod1 = lm(value ~ year, data = df)
#   slopeMod = coef(mod1)[2]
#   return(slopeMod)
# })
# new_data = attr(mods, 'split_labels')
# new_data$var = unlist(mods)
# plot_data = new_data
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data = plot_data, legTitle = 'Slope', palFun = rainbowFunc)
# 
# ggsave(filename = 'figures/fore_slope_hatchsuc.png', device = 'png', width = 100, height = 160, units = 'mm', dpi = 500)
# 
# write.csv(plot_data, 'output_data/fore_slope_hatchsuc.csv', row.names = FALSE)
# 
# # survival:
# plot_data = bind_rows(plot_anom_2, .id = "column_label")
# mods = dlply(plot_data, c("id", "scenario"), function(df) {
#   mod1 = lm(value ~ year, data = df)
#   slopeMod = coef(mod1)[2]
#   return(slopeMod)
# })
# new_data = attr(mods, 'split_labels')
# new_data$var = unlist(mods)
# plot_data = new_data
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data = plot_data, legTitle = 'Slope', palFun = rainbowFunc)
# ggsave(filename = 'figures/fore_slope_surv.png', device = 'png', width = 100, height = 160, units = 'mm', dpi = 500)
# 
# write.csv(plot_data, 'output_data/fore_slope_surv.csv', row.names = FALSE)
# 
# # SL
# plot_data = bind_rows(plot_anom_3, .id = "column_label")
# freqTab = plot_data %>%
#             count(id) # select ID with enough obs for SL and DW
# sel_id = freqTab$id[which(freqTab$n > 100)]
# plot_data = plot_data[plot_data$id %in% sel_id, ]
# 
# mods = dlply(plot_data, c("id", "scenario"), function(df) {
#   mod1 = lm(value ~ year, data = df)
#   slopeMod = coef(mod1)[2]
#   return(slopeMod)
# })
# new_data = attr(mods, 'split_labels')
# new_data$var = unlist(mods)
# plot_data = new_data
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data = plot_data, legTitle = 'Slope', palFun = rainbowFunc)
# ggsave(filename = 'figures/fore_slope_SL.png', device = 'png', width = 100, height = 160, units = 'mm', dpi = 500)
# 
# write.csv(plot_data, 'output_data/fore_slope_SL.csv', row.names = FALSE)
# 
# # DW
# plot_data = bind_rows(plot_anom_4, .id = "column_label")
# plot_data = plot_data[plot_data$id %in% sel_id, ]
# mods = dlply(plot_data, c("id", "scenario"), function(df) {
#   mod1 = lm(value ~ year, data = df)
#   slopeMod = coef(mod1)[2]
#   return(slopeMod)
# })
# new_data = attr(mods, 'split_labels')
# new_data$var = unlist(mods)
# plot_data = new_data
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data = plot_data, legTitle = 'Slope', palFun = rainbowFunc)
# ggsave(filename = 'figures/fore_slope_DW.png', device = 'png', width = 100, height = 160, units = 'mm', dpi = 500)
# 
# write.csv(plot_data, 'output_data/fore_slope_DW.csv', row.names = FALSE)
# 

# Plot 8: Environmental variables -------------------------------------------------

plot_dataf = bind_rows(plot_data_8, .id = "column_label")
plot_datah = bind_rows(plot_data_h8, .id = "column_label")
plot_data = rbind(plot_datah, plot_dataf)
plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
plot_data$variable = factor(plot_data$variable, levels = c("temperature", 'pCO2'))
plot_data$variable2 = factor(plot_data$variable, labels = c("Temperature~(C)", 'pCO[2]~(mu*atm)'))

# Plot:
ggplot(plot_data, aes(x = year)) + 
  geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
  #geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
  geom_line(aes(y = q50, colour = scenario)) +
  theme_bw() +
  xlab(NULL) +
  ylab(NULL) +
  scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2003, to = maxHindYear, by = 2)) +
  scale_color_brewer(palette = mainPal) +
  scale_fill_brewer(palette = mainPal) +
  theme(legend.position = 'none') +
  facet_wrap(. ~ variable2, scales = 'free_y', strip.position = "left", 
           labeller = label_parsed, ncol = 1) +
  theme(strip.background = element_blank(),
        strip.placement = "outside")

ggsave(filename = 'figures/forehind_envvar.png', device = 'png', width = 95, height = 150, units = 'mm', dpi = 500)

write.csv(plot_data, 'output_data/forehind_envvar.csv', row.names = FALSE)


# Plot 9: prey data -------------------------------------------------------

# plot_data = bind_rows(plot_data_9, .id = "column_label")
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# plot_data$variable = factor(plot_data$variable, levels = c("euphausiids",
#                                                            "euphausiidsShelf", "neocalanusShelf", 
#                                                            "neocalanus", "copepods", "microzoo"))
# plot_data$variable2 = factor(plot_data$variable, labels = c("EupO~(mg~C/m^3)",
#                                                             "EupS~(mg~C/m^3)", "NCaS~(mg~C/m^3)", 
#                                                             "NCaO~(mg~C/m^3)", "Cop~(mg~C/m^3)", "MZP~(mg~C/m^3)"))
# 
# # Plot:
# ggplot(plot_data, aes(x = year)) + 
#   geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
#   geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
#   geom_line(aes(y = q50, colour = scenario)) +
#   theme_bw() +
#   xlab(NULL) +
#   ylab(NULL) +
#   scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2015, to = 2095, by = 20)) +
#   scale_color_brewer(palette = mainPal) +
#   scale_fill_brewer(palette = mainPal) +
#   theme(legend.position = 'none') +
#   facet_wrap(. ~ variable2, scales = 'free_y', strip.position = "left", 
#              labeller = label_parsed) +
#   theme(strip.background = element_blank(),
#         strip.placement = "outside")
# 
# ggsave(filename = 'figures/fore_preyvar.png', device = 'png', width = 180, height = 100, units = 'mm', dpi = 500)
# write.csv(plot_data, 'output_data/fore_envprey.csv', row.names = FALSE)
# 
# # -------------------------------------------------------------------------
# # -------------------------------------------------------------------------
# 
# # Information to map
# shift_value_1 <- 0
# shift_value_2 <- 360
# 
# map_world_df <- map_data('world', wrap=c(shift_value_1, shift_value_2)) %>%
#   dplyr::filter(region != "Antarctica")
# 
# country_shapes <-  geom_polygon(data = map_world_df, 
#                                 aes(x=long, y = lat, group = group),
#                                 fill = "gainsboro",
#                                 color = "gainsboro",
#                                 size = 0.15)

# -------------------------------------------------------------------------
# Plot trajectories:

# List to save plots:
# plotList = list()
# indList = 1
# 
# for(i in seq_along(cores)) {
#   
#   core_name = cores[i]
#   files_core = list.files(path = file.path(main_folder, core_name))
#   fcore = grep(pattern = 'Results_files', x = list.files(path = file.path(main_folder, core_name)))
#   
#   for(j in seq_along(fcore)) {
#    
#     tmpData = read_data_in(eggInclude = FALSE, 
#                            path = file.path(main_folder, core_name, files_core[fcore[j]]))
#     tmpData$horizPos1 = ifelse(test = tmpData$horizPos1 > 0, yes = tmpData$horizPos1 - 360, 
#                                no = tmpData$horizPos1)
#     tmpData$horizPos1 = tmpData$horizPos1 + 360
#     tmpData$relDay = lubridate::day(tmpData$startTime)
#     
#     # Find initial and final points
#     init_points = tmpData[tmpData[ , .I[which.min(time)], by = id]$V1]
# 
#     plotList[[indList]] = plot_trajectory(tmpData)
#     
#     indList = indList+1
#      
#   } 
#     
# }
# 
# # Plot trajectories: 
# 
# plotList2 = plotList[c(1,5,8,2,6,9,3,7,10,4)] # reorder list by year
# 
# png(filename = 'figures/hind_trajectories.png', width = 190, height = 200, 
#     units = 'mm', res = 500)
# do.call("grid.arrange", c(plotList2, ncol = 3))
# dev.off()



# Plot spatial indicators -------------------------------------------------

# # Plot : Distance traveled per year: time series
# 
# dist_data = bind_rows(distdat, .id = "column_label")
# dist_data$q50 = dist_data$q50*1.852 # from nm to km
# dist_data$q5 = dist_data$q5*1.852 # from nm to km
# dist_data$q95 = dist_data$q95*1.852 # from nm to km
# dist_data$q25 = dist_data$q25*1.852 # from nm to km
# dist_data$q75 = dist_data$q75*1.852 # from nm to km
# plot_data = dist_data
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# ps2 = ggplot(plot_data, aes(x = year)) + 
#   geom_ribbon(aes(ymin = q5, ymax = q95, fill = scenario), alpha=0.1) +
#   geom_ribbon(aes(ymin = q25, ymax = q75, fill = scenario), alpha=0.2) +
#   geom_line(aes(y = q50, colour = scenario)) +
#   theme_bw() +
#   xlab('') +
#   ylab('Distance (km)') +
#   scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2015, to = 2095, by = 20)) +
#   coord_cartesian(ylim = c(0, 1000)) +  
#   scale_color_brewer(palette = mainPal) +
#   scale_fill_brewer(palette = mainPal) +
#   theme(legend.position = 'none')
# 
# write.csv(plot_data, 'output_data/fore_spatdist.csv', row.names = FALSE)
# 
# # Plot: Distance traveled map
# plot_data = bind_rows(distdat_id, .id = "column_label")
# plot_data = aggregate(list(var = plot_data$dist), list(id = plot_data$id, scenario = plot_data$scenario),
#                       FUN = mean, na.rm = TRUE)
# plot_data$var = sqrt(plot_data$var)
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data, legTitle = 'Distance (km)', palFun = rainbowFunc, 
#              breaks = c(10,20,30), labels = c(100, 400, 900))
#   
# write.csv(plot_data, 'output_data/fore_map_dist.csv', row.names = FALSE)
# 
# # Plot: Direction map
# plot_data = bind_rows(direct_id, .id = "column_label")
# plot_data = aggregate(list(var = plot_data$dist), list(id = plot_data$id, scenario = plot_data$scenario),
#                       FUN = mean, na.rm = TRUE)
# plot_data$lon = baseLocs$horizPos1[match(plot_data$id, baseLocs$id)]
# plot_data$lat = baseLocs$horizPos2[match(plot_data$id, baseLocs$id)]
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_var(plot_data, legTitle = 'Bearing', palFun = rainbowFunc, 
#              breaks = c(0,90,180,270, 360), limits = c(0,360), labels = c('N', 'E', 'S', 'W', 'N'))
# 
# write.csv(plot_data, 'output_data/fore_map_direc.csv', row.names = FALSE)
# 
# # ggsave(filename = 'figures/hind_disttrav.png', device = 'png', width = 180, height = 90, units = 'mm', dpi = 500)
# 
# # Plot: densities final locations:
# plot_data = bind_rows(end_locations, .id = "column_label")
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# 
# plot_map_2d_density(plot_data = plot_data, palFun = rainbowFunc)
# 
# write.csv(plot_data, 'output_data/fore_map_endpoints.csv', row.names = FALSE)
# 
# # Plot inertia per year:
# 
# cgdata = bind_rows(spatdat, .id = "column_label")
# plot_data = cgdata
# plot_data$scenario = factor(plot_data$scenario, levels = scenarioLevels)
# plot_data$diffInertia = ((plot_data$I_end - plot_data$I_ini)/plot_data$I_ini)*100
# 
# # Plot:
# ps3 = ggplot(plot_data, aes(x = year, y = diffInertia, color = scenario)) + 
#   geom_line() +
#   theme_bw() +
#   xlab('') +
#   ylab('Difference in inertia (%)') +
#   scale_x_continuous(expand=c(0, 0), breaks = seq(from = 2015, to = 2095, by = 20)) +
#   scale_color_brewer(palette = mainPal) +
#   theme(legend.position = 'none') 
# 
# # ggsave(filename = 'figures/hind_inertia.png', device = 'png', width = 100, height = 75, units = 'mm', dpi = 500)
# write.csv(plot_data, 'output_data/fore_spatinert.csv', row.names = FALSE)
# 
# png(filename = 'figures/fore_spatialind.png', width = 95, height = 150, 
#     units = 'mm', res = 500)
# grid.arrange(ps2, ps3, nrow = 2)
# dev.off()
